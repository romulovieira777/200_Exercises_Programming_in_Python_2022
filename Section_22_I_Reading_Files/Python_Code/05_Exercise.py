...
with open("../Files/playway.csv") as file:
    ...
    volume_index = header.split(",").index("Volume")

    # Get the volume values
    volume_values = [int(line.split(",")[volume_index]) for line in data]
    ...
    # Get the date values
    date_values = [line.split(",")[date_index] for line in data]

    # Get the max volume date
    max_volume_date = date_values[volume_values.index(max_volume)]

    print(f"Date: {max_volume_date}")
